...
int main(void)
{
  ...
  MX_FATFS_Init();
  /* USER CODE BEGIN 2 */
  BSP_LCD_Init(); /* Initialize LCD display */
  ...
  BSP_LCD_DisplayStringAt(0, 0, (uint8_t *)"Project for embedded systems", CENTER_MODE);

  /* Check if SD card is detected */	
  if(BSP_SD_IsDetected() == SD_PRESENT) {
	  HAL_UART_Transmit(&huart1, (uint8_t *)"SDCARD OK", 9, HAL_MAX_DELAY); /* SD card is present */
	  HAL_UART_Transmit(&huart1, (uint8_t *) "\n\r", 2, HAL_MAX_DELAY);
	  /* ADD TEXT ON DISPLAY THAT SD CARD IS PRESENT IN GREEN AND NOT PRESENT IN RED IN ELSE STATEMENT*/	
  } else {
	  HAL_UART_Transmit(&huart1, (uint8_t *)"SDCARD NOK", 10, HAL_MAX_DELAY); /* SD card is not present*/
	  HAL_UART_Transmit(&huart1, (uint8_t *) "\n\r", 2, HAL_MAX_DELAY);
...
)
{
	...
	BSP_LCD_SetTextColor(LCD_COLOR_CLACK); /* Set color of text to be displayed to balck */
	int xPos = 10;
	int yPos = 10;
	int multiplier = 1;
	int tab = 10;
    ...
    if (res == FR_OK) {
        for (;;) {
            ...
            if (fno.fattrib & AM_DIR) {                    /* It is a directory */
                ...
                HAL_UART_Transmit(&huart1, &path[i], strlen(&path[i]), HAL_MAX_DELAY); /* Send direcotry name to UART */
                HAL_UART_Transmit(&huart1, (uint8_t *) "\n\r", 2, HAL_MAX_DELAY); /* Send new line to UART */
				
				xPos *= multiplier;
				yPos *= mulitplier;
				mulitplier++;
				BSP_LCD_DisplayStringAt(xPos, yPos, "String to print", LEFT_MODE); /* Display directory name to lcd */
